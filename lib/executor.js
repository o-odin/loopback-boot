...
function defineModels(app, instructions) {
  const models = [];
  for (let model of instructions.models) {
    if (Array.isArray(model.config.dataSource)) {
      for (const dataSourceName of Object.keys(app.dataSources)) {
        const currrentModelName = model.name + dataSourceName
        const modelCopy = JSON.parse(JSON.stringify(model))
        modelCopy.name = currrentModelName
        modelCopy.definition.name = currrentModelName
        modelCopy.definition.http.path = currrentModelName
        modelCopy.config.dataSource = dataSourceName
